...
public class EfetuarLoginPMTest {
    ...
    @Test
    public void testBlockPassword3X() {
        UserDAO userDaoMock = mock(UserDAO.class);
        when(userDaoMock.getByName("andre"))
                .thenReturn(new User("andre", "12345", UserType.NORMALUSER));
        ...
        for (int i = 0; i < 3; i++) {
            try {
                ...
                if (i < 3) {
                    ...
                    assertEquals("Password Block", e.getMessage());
                }
            }
        }
    }
    ...
    @Test
    public void testPasswordWrong2X() {
        UserDAO userDaoMock = mock(UserDAO.class);
        when(userDaoMock.getByName("andre"))
                .thenReturn(new User("andre", "12345", UserType.NORMALUSER));
    ...
    @Test
    public void testBlockPassword4X() {
        UserDAO userDaoMock = mock(UserDAO.class);
        when(userDaoMock.getByName("andre"))
                .thenReturn(new User("andre", "12345", UserType.NORMALUSER));
